...
double norm_LbLSC = scaleFactorPhoton*xsecGeneratedLbLSC*luminosity/nEventsGeneratedLbLSC;   
...
*/
  ...
  //      cout << "Muon size:" << treeR.nMu << endl;
        ...
        hdipho_Rapidity_unfo[i]->Fill(treeR.vSum_Rapidity,wt[i]);
  ...
  hdipho_pt[2]->Scale(norm_cep2);   hdipho_Rapidity[2]->Scale(norm_cep2);     hInvmass[2]->Scale(norm_cep2);   hdipho_absRapidity[2]->Scale(norm_cep2);    hInvmass_unfo[2]->Scale(norm_cep2);  hdipho_Rapidity_unfo[2]->Scale(norm_cep2);
  }
...
/*   TCanvas*c20 = new TCanvas();
   ...
   */
     TCanvas*c20 = new TCanvas();
//     c20->SetOptstat(0);
     hSigmaIEta_Eta[1]->GetXaxis()->SetTitle("SigmaIEtaIEta");
     hSigmaIEta_Eta[1]->GetYaxis()->SetTitle("Photon #eta");
     hSigmaIEta_Eta[1]->Draw("COLZ");
       
     c20->Print("SigmaIEta.pdf");   
     TCanvas*c21 = new TCanvas();
   //  hSigmaIEta_Eta2[0]->Draw("COLZ");
 //  TCanvas*c27 = new TCanvas();
  // hAcoplanarity[0]->Draw("PL");
 ...
 //(TCanvas* c1, TH1D* hdata, TH1D* hmc, TH1D* hmc2, double hxmin, double hxmax, double hymin, double hymax, double rymin , double rymax, const char *ytitle, bool iflogy)
  ...
  make_canvas(c2);
  PlotStackHists(c2, hInvmass[0], hInvmass[1], hInvmass[2], hInvmass[3], 0,50,0,30,0.6,1.4,"Invariant Mass", 0);
  ...
  cout << " Invmass at Acop < 0.01 " << Sample[i] <<  " :" << hInvmass[i]->Integral(0,10) << endl;  
...
//TCanvas* PlotStackHists(TCanvas* c1, TH1D* hdata, TH1D* hlbyl, TH1D* hqed, TH1D* hcep,TH1D *hfsr, TH1D *hMG5, TH1D *hMG5_2FSR, double hxmin, double hxmax, double hymin, double hymax, double rymin , double rymax, const char *ytitle, bool iflogy){
 ...
 // hratio->Draw("p");*/
  ...
  cName+=".png";
  c1->SaveAs("Plot/rap_mass2/"+cName);
