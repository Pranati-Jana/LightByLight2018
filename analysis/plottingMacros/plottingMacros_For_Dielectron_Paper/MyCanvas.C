...
public:
					  ...
					  TString DrawOp1 = "EP", TString DrawOp2 = "EPSAME", TString DrawOp_ref = "EPSAME")
  {
    ...
    //TopPad = new TPad("TopPad", "TopPad", 0.01, 0.01, 0.99, 0.99);
     ...
     TopPad = new TPad("TopPad", "TopPad", 0.01, 0.26, 1., 1.);
     TopPad->SetTopMargin(gStyle->GetPadTopMargin()/0.74);
   //  TopPad->SetRightMargin(gStyle->GetPadRightMargin()*0.99);//0.4
      double gTextSize1 = 0.04;
      double gTextSize2 = 0.038*0.9;
      double gYoffset = gStyle->GetTitleYOffset();
    ...
    //TopPad->SetRightMargin(gStyle->GetPadRightMargin()/0.74);
		...
		TopPad->SetRightMargin(gStyle->GetPadRightMargin()*1.2);//*0.74);
		TopPad->SetLeftMargin(gStyle->GetPadLeftMargin()*1.089);//0.98
    ...
    // h2->SetBinError(i, sqrt( pow(h2->GetBinContent(i) * 10, 2) + pow(h2->GetBinError(i), 2) ));
    // }
  
    
    TH1D *h2_ref = (TH1D*) h2->Clone();
     for(int i=0; i <=h2->GetNbinsX()+1; i++){
     h2_ref->SetBinError(i, sqrt( pow(h2->GetBinContent(i) * 0.068, 2) + pow(h2->GetBinError(i), 2) ));
       h2_ref->SetBinContent(i,h2->GetBinContent(i));
     }
    h2_ref->SetFillColor(kBlue+2);
    h2_ref->SetFillStyle(3004);
    h2_ref->SetMarkerSize(0);
    h2_ref->Draw("E2 same");/////
    h2->Draw(DrawOp2);


    TH1D *h1_ref = (TH1D*) h_ref->Clone();
     for(int i=0; i <=h_ref->GetNbinsX()+1; i++){
     h1_ref->SetBinError(i, sqrt( pow(h_ref->GetBinContent(i) * 0.068, 2) + pow(h_ref->GetBinError(i), 2) ));
     h1_ref->SetBinContent(i,h_ref->GetBinContent(i));
     }
    h1_ref->SetFillColor(kRed);
    h1_ref->SetFillStyle(3005);
    h1_ref->SetMarkerSize(0);
    h1_ref->Draw("E2 same");
   ...
   //h_ref->Draw("p same");
    ...
    // -- Y-axis Setting -- //
		h1->GetYaxis()->SetTitleSize(0.0727692);//gTextSize1/(0.55)
    cout << "top pad Y axis title size:" << gTextSize1/(0.55) << endl;
		h1->GetYaxis()->SetLabelSize(gTextSize2/(0.52));
    cout << "top pad Y axis label size:" << gTextSize2/(0.52) << endl;
		h1->GetYaxis()->SetTitleOffset(gYoffset*0.8);
    ...
    legend->SetFillStyle(0);
    legend->SetBorderSize(0);
    //legend->SetHeader("#gamma#gamma#rightarrowe^{+}e^{-}");
    //legend->SetHeaderFont(42, "N"); 
    gStyle->SetTextFont(42);
    legend->SetTextFont(42);
...
latex.DrawLatex(Legend_x1+0.018, Legend_y2 - 0.005 , "#gamma#gamma #rightarrow e^{+}e^{-}");//0.001
     ...
     TLatex *mark = new TLatex();
       ...
       mark->DrawLatex(0.15,startY,"#bf{CMS}");
       mark->DrawLatex(0.49,startY,"#scale[0.8]{PbPb, 1.70 nb^{-1} (#sqrt{s_{NN}} = 5.02 TeV)}");
       mark->Draw();
   ...
   c->cd();
		BottomPad = new TPad("BottomPad","BottomPad",0.01,0.01,0.99,0.3);
      BottomPad->SetTopMargin(0);
      BottomPad->SetRightMargin(0.3);
      BottomPad->SetFillColor(0);
      BottomPad->SetFillStyle(0);
      BottomPad->SetBottomMargin(gStyle->GetPadBottomMargin()*0.74/0.22);
		BottomPad->Draw();
		BottomPad->cd();
    ...
    //BottomPad->SetLeftMargin(0.15);	
		BottomPad->SetRightMargin(gStyle->GetPadRightMargin()*0.96);/*.755*/
		BottomPad->SetLeftMargin(gStyle->GetPadLeftMargin()*1.1);//.99
		if( isRatioPadAttached == kTRUE) BottomPad->SetTopMargin(0.00);
    ...
    for (int i=0; i<=h2_noerr->GetNbinsX()+1; i++) h2_noerr->SetBinError(i,0);
      ...
      // -- Make ratio plot for the h1_ref error
      TH1D *h2_noerr_unit = (TH1D*) h1->Clone();
      TH1D *h2_unit1 = (TH1D*) h_ratio1->Clone();
      TH1D *h2_unit = (TH1D*) h2_ref->Clone();
      h2_unit->Divide(h2_unit);
      for (int i=0; i<=h2_ref->GetNbinsX()+1; i++) {
         if (h2_ref->GetBinContent(i)==0) {
            h2_unit->SetBinError(i,0);
	   }
	 else{
           // h2_unit->SetBinError(i,(h2_ref->GetBinError(i) * h1->GetBinContent(i))/ pow(h2_ref->GetBinContent(i),2));
            h2_unit->SetBinContent(i,1);
           }
      }
      h2_unit->SetFillColor(kBlue+2);
      h2_unit->SetFillStyle(3004);
      h2_unit->SetMarkerSize(0);
      h2_unit->Draw("E2 same");
    ...
    // -- Make Ratio plot with 2nd MC & Draw it -- //
      ...
      // -- Make ratio plot for the h2 error
      TH1D *href_noerr_unit = (TH1D*) h1->Clone();
      TH1D *href_unit1 = (TH1D*) h_ratio2->Clone();
      TH1D *href_unit = (TH1D*) h1_ref->Clone();
       href_unit->Divide(href_unit);
      for (int i=0; i<=h1_ref->GetNbinsX()+1; i++) {
         if (h1_ref->GetBinContent(i)==0) {
            href_unit->SetBinError(i,0);
	 }
	 else{
       //     href_unit->SetBinError(i,(h1_ref->GetBinError(i) *  h1->GetBinContent(i))/ pow(h1_ref->GetBinContent(i),2));
             // href_unit->
     		 href_unit->SetBinContent(i,1);
         }
      }
      href_unit->SetFillColor(kRed);
      href_unit->SetFillStyle(3005);
    ...
    h_ratio1->GetXaxis()->SetTitleOffset( 0.98 );
    h_ratio1->GetXaxis()->SetTitleSize( 0.181818 );//0.22//gTextSize1/0.22 
    cout << "ratio X axis title size:" << gTextSize1/0.22 << endl;
    //h_ratio1->GetXaxis()->SetTitleSize( 0.2 );
    h_ratio1->GetXaxis()->SetLabelColor(1);
    h_ratio1->GetXaxis()->SetLabelFont(42);
    h_ratio1->GetXaxis()->SetLabelOffset(0.02);
    //h_ratio1->GetXaxis()->SetLabelSize(0.15);
      h_ratio1->GetXaxis()->SetLabelSize(gTextSize2/0.21);//0.25
      cout << "ratio X axis label size:" << gTextSize2/0.21 << endl;
      h_ratio1->GetXaxis()->SetNdivisions(505);
    ...
    // -- Y-axis Setting -- //
		...
		h_ratio1->GetYaxis()->SetTitleSize(0.181818);//gTextSize1/0.25
    //cout << "ratio Y axis title size:" << gTextSize1/0.25 << endl; 
 		h_ratio1->GetYaxis()->SetLabelSize(gTextSize2/0.21);
    cout << "ratio Y axis label size:" << gTextSize2/0.21 << endl;
    h_ratio1->GetYaxis()->SetLabelOffset(0.01);
  ...
  //   f_line->SetLineWidth(1);
   // f_line->Draw("SAME");
    ...
    h_ratio1->Draw("EPSAME");
   // h_ratio1->GetYaxis()->SetLabelSize(0);
   // h_ratio1->GetYaxis()->SetNdivisions(50);
   h_ratio1->GetYaxis()->SetNdivisions(505);  // Set initial divisions
//h_ratio1->GetYaxis()->SetLabelSize(0);
